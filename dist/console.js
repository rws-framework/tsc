...
async function transpile({ runspaceDir, entries = { main: 'src/index.ts' }, buildDir = null, outFileName = 'main.js', tsPaths = {}, isDev = false, extraNodeModules = [], dynamicImports = true, dirnameFilenameReplace = true }) {
    try {
        if (!buildDir) {
        ...
        if (doWarmCache) {
            await (0, build_1.buildCLI)(entries, appRoot, runspaceDir, buildDir, outFileName, tscExecDir, tsPaths, isDev, extraNodeModules, hasRebuild, isVerbose, dynamicImports, dirnameFilenameReplace);
        }
        else {
